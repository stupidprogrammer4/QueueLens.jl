# Archived Learning Log

Historical snapshot before completion of milestones 4-6. Statements about
unimplemented features below describe that earlier state, not the current
application. See ../PROGRESS.md and ../README.md for current behavior.
...
Latest default suite: all 5520 checks pass via
`julia --project=. test/runtests.jl`. The 19 completed budget checks now join
the default suite, along with 84 callable-backoff infrastructure checks.
The standalone exponential/jitter contract currently has 7 passes, 2 failures
and 21 errors because its two calculations are learner-owned stubs. Built-in
fixed backoff and custom callables work; retry execution is not enabled.
...
### Current exercise: callable backoff

The learner completed retry_delay's budget logic. At the learner's request,
RetryPolicy now stores a callable backoff rather than a numeric delay. The
assistant preserved budget checks and wired retry_delay(policy, attempt_id,
rng) to validate callback shape and finite nonnegative output. Exhausted and
invalid attempts never invoke backoff. The experimental delay keyword and
two-argument helper were replaced; callers and tests now pass backoff and RNG.

Functions, closures and callable structs work. FixedBackoff is complete;
ExponentialBackoff and FullJitterBackoff have validated configuration and
learner-owned calculation stubs in src/models/backoff.jl. Next is capped
doubling and full jitter using the run's explicit RNG. See docs/retry_exercise.md
and test/retries/backoff_contract_tests.jl. Configuration, callback, RNG and
completed budget checks are in the default suite; calculation contracts remain
separate until implemented. RetryReady, cleanup, reset, terminal history and
retry reporting remain future work; no policy is connected to simulate yet.

### Completed: attempt identity

The assistant prepared `JobRecord.attempt_id`, immutable positive attempt tags
on completion/failure/timeout events, scheduling propagation, and pre-mutation
validation in handlers and bookkeeping. The main loop now consults stale
filtering for every event, before clock advancement, even without terminal ids.
Existing event constructor calls default to attempt 1. Input jobs and outcome
schemas are unchanged; no actual retry, increment, reset or backoff is enabled.

The learner completed `is_stale_event` in `src/simulation/handlers.jl`: older
attempts of live records are ignored, equal-attempt and absent/terminal rules
are preserved, and future attempts reach validation. All 80 contract checks
pass and are now included alongside infrastructure tests in the default suite.
The assistant removed the exercise TODO without changing the learner's logic.
See `docs/attempt_exercise.md` for the completed contract.
...
remains `result.failed` with reason `:timeout`. Retry execution remains pending;
attempt identity is prepared in the current exercise above.
...
Attempt identity is now complete; retry execution remains milestone-4 work.
